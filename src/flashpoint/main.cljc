(ns flashpoint.main
  "Kotodama WASM entrypoint for the Flashpoint clean-room actor (L4) — Clojure port.
  ...
  facts (`flashpoint.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "flashpoint")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"flashpoint.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "flashpoint-compat" :tier tier :entities entities} 200])
...
(defn start! [] :flashpoint-compat/ready)
